...
exports.index = function* () {
  ...
  yield this.render('platform.html',{
    current:"platform",
    title:"平台管理",
...
exports.main = function *(){
  ...
  if(oper === 'add'){
    yield this.service.platform.insert({
    ...
    yield this.service.platform.update({
      id,
      name,
      comment,
      status
    ...
    for(let i =0, l = id.length;i<l; i++){
      ...
      yield this.service.platform.remove(id[i]);
...
exports.list = function* () {
  ...
  if(_search !== 'true'){
    result = yield this.service.platform.list(pageNum, pageSize);
    total = yield this.service.platform.count('1=1');
  }else{
    result = yield this.service.platform.search(pageNum, pageSize, sql);
    total = yield this.service.platform.count(sql);
  }
