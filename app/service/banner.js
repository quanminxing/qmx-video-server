...
class BannerService extends Service {
    async list(param) {
        ...
        let results = await this.app.mysql.query(`select A.*, date_format(A.timestamp,'%Y-%m-%d %H:%i') as timestamp from video_banner AS A left join video_banner_type AS B on A.type_id = B.id where A.is_show = 1  = false and ${cond};`);
