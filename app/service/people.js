...
class PeopleService extends Service {
  ...
  async list(pageNum, pageSize) {
    const articles = await this.app.mysql.query('select id,name,cname,description,phone,auth,password,timestamp from video_worker order by timestamp desc limit ? offset ?;', [pageSize, (pageNum - 1) * pageSize]);
    return articles;
  }

  async listAll() {
    const articles = await this.app.mysql.query('select id,name,cname,description,phone,auth,password,timestamp from video_worker order by timestamp desc ');
    return articles;
  }
  ...
  // 总数
  async count(where) {
    const count = await this.app.mysql.query('select count(*) from video_worker where ?', [where]);
